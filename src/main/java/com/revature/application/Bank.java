...
public class Bank {
	public static void main(String[] args) throws Exception {
		...
		try {			
			users = userService.createUser(users).get();
			
			if(userService.login(users).get()) {
				System.out.println(users.getUsername() + " successfully logged in");
			}

			//System.out.println(users);
		} catch (NoSuchElementException e) {
			System.out.println("Unable to fetch the users");
		}
	}
}
